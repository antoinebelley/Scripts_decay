...
class Decay():
  ...
  def send_queue(self, time):
    ...
    if str(os.environ['HOSTNAME']) == 'oak.arc.ubc.ca':
        ...
        submit  = f'python {imasms}nuqsub.py {command} {self.nucI} {self.run_name} -m 125 -t {time}'
    os.system(submit)
...
class NutbarDirM2nu(NutbarDir):
  ...
  instances = []
  def __init__(self, directory, mydir, filebase, outfile, step, JI=0, NI=1, JF=0, NF=1, MEC = False, inducedforce = False):
    NutbarDir.__init__(self, directory, mydir, filebase, outfile, step, JI, NI, JF, NF)
    self.__class__.instances.append(weakref.proxy(self))
    if MEC == True:
      if inducedforce == True:
        self.op1 = f'{self.filebase}_GTMECcD2_1b.op'
        self.op2 = f'{self.filebase}_GTMECcD2_2b.op'
      else:
        self.op1 = f'{self.filebase}_GTMEC_1b.op'
        self.op2 = f'{self.filebase}_GTMEC_2b.op'
    else:
      if inducedforce == True:
        self.op1 = f'{self.filebase}_GTMECif_1b.op'
        self.op2 = f'{self.filebase}_GTMECif_2b.op'
...
class NutbarDirDGT(NutbarDir):
  """Class describing the common attributes of 
  a NutbarlDir of the 0vbb"""
  instances = []
  def __init__(self, directory, mydir, filebase, outfile, step, path, JI=0, NI=1, JF=0, NF=1):
    NutbarDir.__init__(self, directory, mydir, outfile, step, JI, NI, JF, NF)
    self.path     = path
    self.op1      = f'*{filebase}_1b.op'
    self.op2      = f'*{filebase}_2b.op'
    self.__class__.instances.append(weakref.proxy(self))
...
class M2nu(Decay):
  ...
  def __init__(self, Z, A, flow, sp, inter, int3N, emax, hw, 
               neigK, BB, MEC = False, indfor = False):
    Decay.__init__(self, Z, A, flow, sp, inter, int3N, emax, hw)
    ...
    if indfor == True and MEC == False:
      self.mydir = f'{self.mydir}_IF'
    #Directory for the symlink file
    self.linkdir  = f'{self.mydir}/nushx_symlinks'   # this directory just holds the symlinks script and output from its clutser run
    self.MEC      = MEC
    ...
    if MEC == True:
      self.run_name = f'{self.run_name}_MEC'
    if indfor == True and MEC == False:
      self.run_name = f'{self.run_name}_IF'
    ...
    self.filebase = f'{filebase}e{self.emax}*hw{self.hw}*A{self.A}*2*'
    self.intfile  = f'{filebase_int}e{self.emax}*hw{self.hw}*A{self.A}*2*_.int'
    self.spfile   = f'{filebase_int}e{self.emax}*hw{self.hw}*A{self.A}*2*_.sp'
    #Create instances of the SubDirectories to add in mydir
    #MAKE SURE THIS RESCPECT THE NAMING CONVENTION`
    ...
    self.KIdir    = NutbarDirM2nu(directory ='KI_nutbar', mydir = self.mydir, filebase = self.filebase, outfile = outfileKI, step ='KI', JF=self.sesJK, NF=self.neigK, MEC = MEC, inducedforce = indfor)
    self.FKdir    = NutbarDirM2nu(directory ='FK_nutbar', mydir = self.mydir, filebase = self.filebase, outfile = outfileFK, step ='FK', JI=self.sesJK, NI=self.neigK, MEC = MEC, inducedforce = indfor)
    #Assign the list of the NushellDir and NutbarDir names to the
    #appropriate lists
    self.nush_list = NushellDir.instances
...
class DGT(Decay):
  def __init__(self, Z, A, flow, sp, inter, int3N, emax, hw, BB):
    Decay.__init__(self, Z, A, flow, sp, inter, int3N, emax, hw)
    self.decay    = 'DGT'
    self.BB       = BB
    self.mydir    = Decay.mydir(self) #Adds the DGT at the beginning of the one coming from Decay.__init__
    self.run_name = f'{self.decay}_{self.flow}_{self.sp}_{self.int}_{self.int3N}_e{self.emax}_hw{self.hw}'
    self.imaout   = f'/home/belleya/projects/def-holt/belleya/imsrg/work/output_{self.nucI}_DGT/'
    #Create instances of the SubDirectories to add in mydir
    #MAKE SURE THIS RESCPECT THE NAMING CONVENTION`
    #Create NushellDir instances
    self.nudirI   = NushellDir('nushxI_data', self.mydir, self.nucI, self.ZI, self.neigI, 0, self.maxJI, self.delJI)
    self.nudirF   = NushellDir('nushxF_data', self.mydir, self.nucF, self.ZF, self.neigF, 0, self.maxJF, self.delJF)
    #Regex to find the sp/int and op file later on. 
    #Op file are given as filebase in NutDir
    if self.flow == 'BARE':
      filebase = f'*{self.flow}*{self.BB}*'
    else:
      filebase    = f'*{self.int}*{self.BB}*'
    self.filebase = f'{filebase}e{self.emax}*hw{self.hw}*A{self.A}*_DGT*'
    self.intfile  = f'{self.filebase}*_.int'
    self.spfile   = f'{self.filebase}*_.sp'
    outfile       = f'nutbar_tensor0_{self.nucF}0.dat'
    #Create NutbarDir instance
    self.FIdir = NutbarDirDGT('FI_nutbar', self.mydir, self.filebase, outfile, step = 'FI', path = self.imaout)
    #Assign the list of the NushellDir and NutbarDir names to the
    #appropriate lists
    self.nush_list = NushellDir.instances
    self.nut_list  = NutbarDirDGT.instances
  ...
  def write_sh(self):
    """Write a bash script to execute execute_M2nu.py"""
    if str(os.environ['HOSTNAME']) == 'oak.arc.ubc.ca':
      file = '/global/home/belley/Scripts_decay/execute_DGT.py'
      sh   = '/global/home/belley/Scripts_decay/execute_DGT.sh'
    elif str(os.environ['HOSTNAME'])[7:] == 'cedar.computecanada.ca':
      file = '/home/belleya/projects/def-holt/belleya/Scripts_decay/execute_DGT.py'
      sh   = '/home/belleya/projects/def-holt/belleya/Scripts_decay/execute_DGT.sh'
    else:
      print('This cluster is not known')
      print('Add the paths to execute_M2nu.py in this cluster')
      print('Exiting...')
    ...
    f.write('#!/bin/bash\n\n')
    f.write(f'chmod +x {file}\n')
    f.write(f'python {file} {self.nucI} {self.nucF} {self.run_name}')
    f.close()
    st = os.stat(sh)
    os.chmod(sh,st.st_mode |stat.S_IXUSR| stat.S_IXGRP|stat.S_IXOTH )
    return sh
