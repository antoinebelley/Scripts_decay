...
def calc_me_0vbb(name,kshl_dir, fn_snt, fn_0v_snt, fn_ptn_bra, fn_ptn_ket, fn_wf_bra, fn_wf_ket, fn_density=None, \
        header="", batch_cmd=None, run_cmd=None, fn_input="transit.input"):
    if(fn_density==None):
        basename = os.path.basename(fn_snt)
        fn_out = "calc_me_0vbb_" + name + ".dat"
    if(fn_density!=None): fn_out = fn_density
    fn_script = os.path.splitext(fn_out)[0] + ".sh"
    cmd = "cp " + kshl_dir + "/transit.exe ./"
    subprocess.call(cmd,shell=True)
    ...
    prt += 'EOF\n'
    from Op import Op
    nme = Op(kshl_dir+fn_0v_snt, rankJ=0, rankP=1, rankZ=-2)
...
def main():
    ...
    sys.argv[2] = kshl_dir
    sys.argv[3] = fn_snt
    sys.argv[4] = fn_0v_snt
    sys.argv[5] = fn_ptn_bra
    sys.argv[6] = fn_ptn_ket
    sys.argv[7] = fn_wf_bra
    sys.argv[8] = fn_wf_ket
    calc_me_0vbb(name,kshl_dir, fn_snt, fn_0v_snt, fn_ptn_bra, fn_ptn_ket, fn_wf_bra, fn_wf_ket)
