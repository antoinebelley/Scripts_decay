...
def calc_me_0vbb(exe_dir, fn_snt, fn_0v_snt, fn_ptn_bra, fn_ptn_ket, fn_wf_bra, fn_wf_ket, fn_density=None, \
        header="", batch_cmd=None, run_cmd=None, fn_input="transit.input"):
    kshl_dir = exe_dir
    #exe_dit = "/home/belleya/projects/def-holt/shared/KSHELL/version-2019-08-15/"
    if(fn_density==None):
        basename = os.path.basename(fn_snt)
        fn_out = "calc_nme_0vbb_" + name + ".dat"
    if(fn_density!=None): fn_out = fn_density
    fn_script = os.path.splitext(fn_out)[0] + ".sh"
    cmd = "cp /home/belleya/projects/def-holt/shared/KSHELL/version-2019-08-15/transit.exe ./"
    subprocess.call(cmd,shell=True)
    prt = header + '\n'
    prt += 'echo "start runnning ' + fn_out + ' ..."\n'
    prt += 'cat >' + fn_input + ' <<EOF\n'
    prt += '&input\n'
    prt += '  fn_int   = "' +kshl_dir+ fn_snt + '"\n'
    prt += '  fn_ptn_l = "' +kshl_dir+ fn_ptn_bra + '"\n'
    prt += '  fn_ptn_r = "' +kshl_dir+ fn_ptn_ket + '"\n'
    prt += '  fn_load_wave_l = "' +kshl_dir+ fn_wf_bra + '"\n'
    prt += '  fn_load_wave_r = "' +kshl_dir+ fn_wf_ket + '"\n'
    prt += '  hw_type = 2\n'
    prt += '  eff_charge = 1.5, 0.5\n'
    prt += '  gl = 1.0, 0.0\n'
    prt += '  gs = 3.91, -2.678\n'
    ...
    if(run_cmd == None):
        prt += './transit.exe ' + fn_input + ' > ' + fn_out + ' 2>&1\n'
    if(run_cmd != None):
        prt += run_cmd +" "+kshl_dir+'transit.exe ' + fn_input + ' > ' + fn_out + ' 2>&1\n'
    #prt += 'rm ' + fn_input + '\n'
    f = open(fn_script,'w')
    f.write(prt)
    f.close()
...
def main():
    name = sys.argv[1]  
    fn_snt = sys.argv[2]
    fn_0v_snt = sys.argv[3]  
    fn_ptn_bra = sys.argv[4] 
    fn_ptn_ket = sys.argv[5] 
    fn_wf_bra = sys.argv[6] 
    fn_wf_ket = sys.argv[7]
    calc_me_0vbb(name, fn_snt, fn_0v_snt, fn_ptn_bra, fn_ptn_ket, fn_wf_bra, fn_wf_ket)
