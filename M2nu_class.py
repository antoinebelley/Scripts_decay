...
class Directory():
  ...
  def __init__(self, nucI, nucK, nucF, BB, inter, emax, hw, neigK, shift, correction, MEC = False, IF = False):
    ...
    if BB == 'OS':
      ...
      directory = f'{nucI}/M2nu*_3N_*{inter}*e{emax}*hw{hw}*neig{neigK}'
    else:
    ...
    if BB != 'HF':
      ...
      self.KIdir   = 'KI_nutbar_HF'
      self.FKdir   = 'FK_nutbar_HF'
    self.KIdirMec  = 'KI_nutbar_Mec'
    self.FKdirMec  = 'FK_nutbar_Mec'
    self.KIdirIF   = 'KI_nutbar_If'
    self.FKdirIF   = 'FK_nutbar_If'
  ...
  def sumNME(self):
    ...
    for i in range(7,maxKsum-1):
      ...
      if self.correction == 'corrected' and i == 7:
      ...
      if self.MEC == True:
        ...
        EK_MEC, nmeKI_MEC = read_Eses(state = i, directory = f'{self.path}/{self.KIdirMec}', file = self.nutfileK)
        EF_MEC, nmeFK_MEC = read_Eses(state = i, directory = f'{self.path}/{self.FKdirMec}', file = self.nutfileF)
        nmeKI_MEC *= factor
        nmeFK_MEC *= factor
        Mexc  = single_M2nu(nmeKI_MEC, nmeFK_MEC, Mexc ,denom)
        Mecs.append((Mexc))
        ...
        if self.IF == True:
          ...
          EK_IF, nmeKI_IF = read_Eses(state = i, directory = f'{self.path}/{self.KIdirIF}', file = self.nutfileK)
          EF_IF, nmeFK_IF = read_Eses(state = i, directory = f'{self.path}/{self.FKdirIF}', file = self.nutfileF)
          nmeKI_IF *= factor
          nmeFK_IF *= factor
          ...
          M2nu  = single_M2nu(nmeKI_IF, nmeFK_IF, M2nu ,denom)
          #END OF IF STATEMENT FOR INDUCED FORCE
        else:
          M2nu  = single_M2nu(nmeKI_MEC, nmeFK_MEC, M2nu ,denom)
          self.Mecs = Mecs[len(Mecs)-1]
        #END OF IF STATEMENT FOR Mecs
      else:
...
class DataM2nu():
  ...
  def __init__(self, Z, A, inter, emax, hw, neigK, shift, correction):
    ...
    if inter == 'N4LO':
      self.IfDir     = Directory(self.nucI, self.nucK, self.nucF, '3N', inter, emax, hw, neigK, shift, correction, MEC=True, IF = True)
